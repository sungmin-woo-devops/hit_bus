...
class UserDatabase:
    def __init__(self, use_cloud=None, host=None, user=None, password=None, database=None):
        """
        데이터베이스 연결 초기화
        ...
        """
        # Flask 앱 컨텍스트에서 설정 가져오기
        if current_app:
            self.use_cloud = use_cloud if use_cloud is not None else current_app.config.get('DATABASE_USE_CLOUD', True)
            self.host = host or current_app.config.get('DATABASE_HOST', 'svc.sel5.cloudtype.app')
            self.port = current_app.config.get('DATABASE_PORT', 32134)
            self.user = user or current_app.config.get('DATABASE_USER', 'root')
            self.password = password or current_app.config.get('DATABASE_PASSWORD', '1111')
            self.database = database or current_app.config.get('DATABASE_NAME', 'mysql')
        else:
            # 기본 설정
            self.use_cloud = use_cloud if use_cloud is not None else True
            self.host = host or 'svc.sel5.cloudtype.app'
            self.port = 32134
            self.user = user or 'root'
            self.password = password or '1111'
    ...
    def create_user(self, username: str, email: str, password: str, full_name: str, 
                   phone: Optional[str] = None, birth_date: Optional[datetime] = None) -> int:
        """새 사용자 등록"""
        try:
            # 비밀번호 해시화
